...
👤 내 역할 (김성준)
CRUD 연동 · 페이지 구조 설계 · Git 형상관리
구조 설계

App.jsx 라우팅 구조 설계 및 구현 — 페이지 간 이동 흐름 정의
컴포넌트 폴더 구조 설계 (screen/, hooks/, common/ 등) — 팀원들이 일관된 구조 안에서 개발할 수 있도록 기반 구축

CRUD 구현

도서 관련 전체 페이지 담당 — 목록 조회 / 상세 조회 / 등록 / 수정 / 삭제
axios 기반 API 연동 및 커스텀 훅 구현
PATCH 방식 부분 업데이트, 삭제 전 Confirm 알림 등 UX 처리 포함

Git 형상관리

PR 리뷰 전담 — 전체 팀원 코드 리뷰 및 병합 관리
모든 충돌 해결 담당
팀원들이 충돌 가능성을 인지하지 않은 채 타인 코드를 수정하는 상황이 반복되어 충돌 해결 난이도가 높았음 → 브랜치 운용 방식과 작업 범위 가이드를 직접 수립하여 후반부 충돌 빈도를 줄임

개발 환경 세팅

팀원 전원 로컬 환경 세팅 지원 (Node.js, yarn, json-server, Vite 설정)
